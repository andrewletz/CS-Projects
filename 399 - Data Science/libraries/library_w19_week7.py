...
def get_oob_table(original_table, forest):
...
def euclidean_distance(vector1, vector2):
  if len(vector1) != len(vector2): 
    raise ValueError('Euclidean distance could not be calculated: vectors must be same length')
...
def knn(row_index, table, k, columns, target):
  row = table.loc[row_index, columns] # get target row
  row_list = list(row)
  
  # get all rows except for row_index
  other_rows = table.loc[table.index != row_index, columns]
  
  # calculate distances for each row
  distances = []
  for idx, other_row in other_rows.iterrows():
    distances.append([idx, euclidean_distance(row_list, list(other_row))])
  
  votes = {0:0, 1:0}
  distances = sorted(distances, key = lambda tup: tup[1])
  for i in range(k): # for each closest row
    curr_row = table.loc[distances[i][0]] 
    votes[curr_row[target]] += 1 # add its 'prediction' to the votes
  
  return 1 if votes[1] > votes[0] else 0

def knn_tester(table, k, columns, target):
  scratch_table = pd.DataFrame(columns=['prediction', 'actual'])
  scratch_table['prediction'] = table.apply(lambda row: knn(row.name, table, k, columns, target), axis = 1)
  scratch_table['actual'] = table[target]
  cases = scratch_table.apply(lambda row: predictor_case(row, pred='prediction', target='actual'), axis=1)
  vc = cases.value_counts()
  return accuracy(vc)
