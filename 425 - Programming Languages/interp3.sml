...
and result
  ...
  | RES_CLOSURE of (result * (string->result))
...
*)
fun interp_lazy_static (env:env, AST_ID i)          = (case env of (Env e)=> (case e(i) of RES_TEMP(ee, enn) => interp_lazy_static(Env(enn), ee) | _=>RES_ERROR "Environment Error"))
  | interp_lazy_static (env:env, AST_NUM n)         = RES_NUM(n)
  | interp_lazy_static (env:env, AST_BOOL b)        = RES_BOOL(b)
  | interp_lazy_static (env:env, AST_FUN (i,e))     = RES_CLOSURE(RES_FUN(i, e), case env of (Env e2)=> e2)
  | interp_lazy_static (env:env, AST_APP (e1,e2))   = let
		val func = interp_lazy_static(env, e1)
	in
		case func of RES_SUCC =>
			(case interp_lazy_static(env, e2) of RES_NUM(n) => RES_NUM(n+1)|_=>RES_ERROR "Argument mismatch, not of type int.")
		| RES_PRED => (case interp_lazy_static(env, e2) of RES_NUM(n) => if n = 0 then RES_NUM(0) else RES_NUM(n-1)|_=>RES_ERROR "Argument mismatch, not of type int.")
		| RES_ISZERO => (case interp_lazy_static(env, e2) of RES_NUM(n)=>if n = 0 then RES_BOOL(true) else RES_BOOL(false) |_=>RES_ERROR "Argument mismatch, not of type int.")
		| RES_CLOSURE(RES_FUN(i,e), env1) => (let
							val kt = RES_TEMP(e2, case env of Env v => v);
							val edd = update (Env(env1)) i kt; 
						in
							interp_lazy_static(Env(edd),e)
						end)
		| _=> RES_ERROR "Argument mismatch, can't apply to non-function"
	end
  | interp_lazy_static (env:env, AST_SUCC)          = RES_SUCC
  | interp_lazy_static (env:env, AST_PRED)          = RES_PRED
  | interp_lazy_static (env:env, AST_ISZERO)        = RES_ISZERO
  | interp_lazy_static (env:env, AST_IF (e1,e2,e3)) = let
		val hyp = interp_lazy_static(env, e1)
	in
		case hyp of RES_BOOL(i)=>if i then interp_lazy_static(env, e2) else interp_lazy_static(env, e3)
