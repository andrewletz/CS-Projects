...
void normalize(double *inputArr, double *targetArr, int len) {
	// calculate norm
	double sum = 0;
	for (int i = 0; i < len; i++) sum += (inputArr[i] * inputArr[i]);
	double norm = sqrt(sum);

	// return new array with all values normalized
	for (int i = 0; i < len; i++) targetArr[i] = inputArr[i] / norm;
}
...
class Triangle {
	public:
		...
		*/
		...
		// normals[vertexId][dimension], where dimension is [x=0, y=1, z=2]
		double normals[3][3];
		double shading[3];
		...
		void splitAndRasterize(Screen *screen) {
			...
			goingUp.shading[0] = this->shading[v1];
			goingUp.shading[1] = this->shading[v2];

			//interpolate final shading value
			goingUp.shading[2] = shading[v1] + proportion * (shading[v3] - shading[v1]);

			// first copy the unchanging colors and normals from the corresponding vertices
			...
			// interpolate color and normal for the final vertex
			...
			goingDown.shading[0] = this->shading[v3];
			goingDown.shading[1] = this->shading[v2];
			...
			goingDown.shading[2] = shading[v1] + proportion * (shading[v3] - shading[v1]);
		...
		void rasterizeTriangle(Screen *screen, int rowMin, int rowMax) {
			...
			for (int row = rowMin; row <= rowMax; row++) {
				...
				// interpolate shadingLeft
				shadingLeft = shading[v1] + proportion * (shading[v2] - shading[v1]);
				...
				// interpolate shadingRight
				shadingRight = shading[v3] + proportion * (shading[v2] - shading[v3]);
				...
				for (int col = (int) ceil_441(leftEnd); col <= (int) floor_441(rightEnd); col++) {
					...
					currentShading = shadingLeft + proportion * (shadingRight - shadingLeft);
					...
					if (currentZ > screen->getZ(col, row)) {
						...
						currentColor[0] = std::min(1.0, currentColor[0] * currentShading);
						currentColor[1] = std::min(1.0, currentColor[1] * currentShading);
						currentColor[2] = std::min(1.0, currentColor[2] * currentShading);
		...
		double calculatePhongShading(double *viewDirection, double *normal) {
			double shadingAmount = 0;
			double normalizedV[3];
			normalize(viewDirection, normalizedV, 3);

			// Ambient term
			shadingAmount += lp.Ka;

			// Diffuse term
			double L_dot_N = lp.lightDir[0] * normal[0] + lp.lightDir[1] * normal[1] + lp.lightDir[2] * normal[2];
			double abs_ldn = (L_dot_N >= 0) ? L_dot_N : L_dot_N * -1;
			shadingAmount += (lp.Kd * abs_ldn);

			// Specular term
			double R[3] = { 2 * L_dot_N * normal[0] - lp.lightDir[0],
							2 * L_dot_N * normal[1] - lp.lightDir[1],
							2 * L_dot_N * normal[2] - lp.lightDir[2] };
			normalize(R, R, 3);
			double V_dot_R = normalizedV[0] * R[0] + normalizedV[1] * R[1] + normalizedV[2] * R[2];
			V_dot_R = (V_dot_R < 0) ? 0 : V_dot_R; // max(0, V_dot_R)

			double specular = lp.Ks * pow(V_dot_R, lp.alpha);
			shadingAmount += specular;

			return shadingAmount;
		}

		// this should be called in world space (ie before camera transforms)
		void calculateShading(Screen *screen) {
			double *cameraPos = screen->currentCamera->position;

			double viewDirection0[3] = {cameraPos[0] - X[0], cameraPos[1] - Y[0], cameraPos[2] - Z[0]};
			double viewDirection1[3] = {cameraPos[0] - X[1], cameraPos[1] - Y[1], cameraPos[2] - Z[1]};
			double viewDirection2[3] = {cameraPos[0] - X[2], cameraPos[1] - Y[2], cameraPos[2] - Z[2]};

			this->shading[0] = calculatePhongShading(viewDirection0, this->normals[0]);
			this->shading[1] = calculatePhongShading(viewDirection1, this->normals[1]);
			this->shading[2] = calculatePhongShading(viewDirection2, this->normals[2]);
		}
...
int main() {
	...
	// code to output only frame000
	// ###############################################
	Screen screen(1000, 1000);

	Camera current_cam = GetCamera(00, 1000);
	screen.currentCamera = &current_cam;
	...
	for (int i = 0; i < triangles.size(); i++) triangles[i].calculateShading(&screen);

	applyAndRasterize(&screen, triangles /* passed by value */, final_matrix);
	WriteImage(screen.image, "frame000");
	// ###############################################

	// code to output movie files, must have subdirectory "movie" created
	...
	// 	if (i < 10) {
	// 		file_name = "movie/frame00" + std::to_string(i);
	// 	} else if (i < 100) {
	// 		file_name = "movie/frame0" + std::to_string(i);
	// 	} else {
	// 		file_name = "movie/frame" + std::to_string(i);
	// 	}
